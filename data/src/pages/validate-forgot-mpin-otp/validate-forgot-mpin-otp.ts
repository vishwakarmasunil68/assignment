...
import {ResetMpinPage} from "../reset-mpin/reset-mpin";

/**
 * Generated class for the ValidateForgotMpinOtpPage page.
 *
...
@IonicPage()
@Component({
  selector: 'page-validate-forgot-mpin-otp',
  templateUrl: 'validate-forgot-mpin-otp.html',
})
export class ValidateForgotMpinOtpPage {
  ...
  callAPI(): void {
    if (this.otp != '') {
      ...
      var url = Constants.VALIDATE_MPIN_OTP;
      ...
      this.http.post(url, postdata).subscribe(result => {
        ...
        try {
          ...
          if (parsedData.status == "success") {
            ...
            this.navCtrl.push(ResetMpinPage);
          } else {
            this.presentToast(parsedData.message);
          }
        } catch (err) {
