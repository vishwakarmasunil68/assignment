...
export class AssignmentDetailsPage {
  ...
  constructor(public navCtrl: NavController, public navParams: NavParams,
    ...
    , public events: Events, public loadingCtrl: LoadingController, public toastCtrl: ToastController, public http: HttpClient, private fileTransfer: FileTransfer,
              private fileChooser: FileChooser, private filePath: FilePath, private iab: InAppBrowser,private iosPicker:IOSFilePicker) {
  }
      ...
      try {
        ...
        if (parsedData.status == "success") {
          ...
          for (let i = 0; i < parsedData.return_data.order_comment.length; i++) {
            ...
            console.log("attachment_url111:-"+parsedData.return_data.order_comment[i].attachment_url);
      ...
      this.http.post(url, postdata).subscribe(result => {
        ...
        try {
          ...
          if (parsedData.status == "success") {
            ...
            // console.log("attachment_url:-"+parsedData.return_data[0].attachment_url);
            // if(parsedData.return_data[0].attachment_url=="null"){
  ...
  chooseFile(): void {
    ...
    //     var currentName = filePath.substr(filePath.lastIndexOf('/') + 1);
    //     this.addMultipartAssignment(encodeURI(filePath), currentName);
    //   })
    //   .catch(e => console.log(e));
  }
      ...
      .then((data) => {
        ...
        try {
          ...
          if (parsedData.status == "success") {
            ...
            console.log("attachment_urlupload:-"+parsedData.return_data[0].attachment_url);
            let is_attachment = false;
            if (parsedData.return_data[0].attachment_url != null && parsedData.return_data[0].attachment_url != "") {
              is_attachment = true;
            }
